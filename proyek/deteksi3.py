...
# Get default resolution of the camera
frame_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
frame_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
print(f"Resolusi kamera: {frame_width}x{frame_height}")

# Optional: Set camera resolution (Uncomment if needed)
...
while True:
    ...
    if key == ord('q'):  # Quit if 'q' is pressed
        ...
        cv2.putText(frame, f"Kapsul: {kapsul_count}  Kemasan: {kemasan_count}", (10, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2, cv2.LINE_AA)  # Font scale 1, thickness 2
