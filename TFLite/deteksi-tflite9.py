...
def detect_capsules_with_tflite(frame, interpreter, input_details, output_details, labels, min_conf=0.5):
    ...
    # Validasi dengan filter warna
    hsv_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

    # Rentang warna kapsul (merah dan oranye)
    ...
    kapsul_count_filtered = 0
    for contour in contours:
        area = cv2.contourArea(contour)
        if area > 100:  # Filter area kecil
            x, y, w, h = cv2.boundingRect(contour)

            # Validasi apakah kontur ini berada dalam bounding box TFLite
            for xmin, ymin, xmax, ymax in detected_boxes:
                if x > xmin and y > ymin and (x + w) < xmax and (y + h) < ymax:
                    kapsul_count_filtered += 1
                    # Gunakan warna yang sama (misalnya hijau) untuk bounding box dari filter warna
                    cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)  # Warna hijau
                    break

    # Total kapsul dihitung berdasarkan kombinasi (model dan filter warna)
    kapsul_count = max(kapsul_count, kapsul_count_filtered)
