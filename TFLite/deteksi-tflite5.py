...
def tflite_detect_realtime(modelpath, lblpath, min_conf=0.5):
    ...
    while True:
        ...
        cv2.imshow('Sistem Pendeteksi Kekurangan Obat Kapsida HS', frame)
        ...
        if key == ord('q'):  # Quit
            ...
            # Calculate missing capsules
            expected_kapsul = count_kemasan * 12
            missing_kapsul = max(0, expected_kapsul - count_kapsul)

            # Print the results
            print(f"Captured Frame:")
            print(f"  Detected Kemasan: {count_kemasan}")
            print(f"  Detected Kapsul: {count_kapsul}")
            print(f"  Missing Kapsul: {missing_kapsul}")

            # Speak the results
            if missing_kapsul > 0:
                speak_message(f"Kurang {missing_kapsul}")
            else:
                speak_message("Sempurna")
